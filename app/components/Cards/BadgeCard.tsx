...
export default function BadgeCard({badge, completed, progress}: {badge: IBadge, completed: boolean, progress?: number}) {
  return (
    <div className="flex items-center relative rounded-xl bg-gray-100 " title={badge.description}>
      <div className="w-32 h-28 flex flex-col items-center gap-2">
        ...
        <div className="flex flex-col leading-[4px]">
          ...
          <span className='text-center text-[10px] leading-[8px]'>{progress ? `${progress > badge.toCompleted ? badge.toCompleted : progress}` : 0} / {badge.toCompleted}</span>
        </div>
      </div>
    </div>
  )
